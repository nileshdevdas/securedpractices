...
@RestController
@RequestMapping(path = "/fileupload/solution")
public class FileUploadSolution {
		...
		public String upload(@RequestParam(value="file") MultipartFile request,@RequestPart(value="filevo") FileUploadVo filevo) throws FileUploadException {
			try {
				...
				if(!FilUploadHelper.isNullOrEmpty(request)) {
					...
					if(!FilUploadHelper.isNullOrEmpty(filevo.name)&&request.getOriginalFilename().matches("^.*\\.(pdf|PDF)$")&&request.getSize()<MaxSize) {
